...
function submitUsername() {
    let username = document.getElementById('forgotUsername').value.trim(); // รับชื่อผู้ใช้จากฟอร์ม
    let users = JSON.parse(localStorage.getItem('users')) || []; // ดึงข้อมูลจาก localStorage

    console.log("users in localStorage:", users); // แสดงข้อมูลผู้ใช้จาก localStorage เพื่อให้เห็นว่าเป็นแบบไหน
    ...
    // ค้นหาผู้ใช้ที่กรอกชื่อผู้ใช้ (ไม่สนใจตัวพิมพ์ใหญ่-เล็ก)
    ...
    if (user) {
        ...
        // หากไม่พบ Username
        document.getElementById('message').textContent = "ไม่พบ Username ในระบบ.";
    }
}

// ฟังก์ชันสำหรับการเปลี่ยนรหัสผ่าน
function resetPassword() {
    let username = document.getElementById('forgotUsername').value.trim(); // รับชื่อผู้ใช้
    ...
    // อัปเดตรหัสผ่าน
    let userIndex = users.findIndex(user => user.username.toLowerCase() === username.toLowerCase());
